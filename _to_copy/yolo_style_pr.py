...
class YOLOStylePR(BaseMetric):
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            gt['gt_instances'] = data_sample['gt_instances']
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        ious = coco_eval.params.iouThrs           # array([0.50, 0.55, ..., 0.95])
    
        # Get mAP@50
        prec_at_50 = precisions[0, :, :, 0, -1]  # IoU=0.50, area=all, maxDets=100
        prec_at_50 = prec_at_50[prec_at_50 > -1]
        mAP_50 = np.mean(prec_at_50) if prec_at_50.size else float('nan')

        # Get mAP@[50:95]
        all_aps = []
        for i in range(len(ious)):
            prec = precisions[i, :, :, 0, -1]
            prec = prec[prec > -1]
            if prec.size:
                all_aps.append(np.mean(prec))

        mAP_50_95 = np.mean(all_aps) if all_aps else float('nan')
        eval_results['mAP_50'] = round(mAP_50, 3)
        eval_results['mAP_50_95'] = round(mAP_50_95, 3)
        print(f'[COCO metrics] mAP_50:{mAP_50}')    
        print(f'[COCO metrics] mAP_50_95:{mAP_50_95}')    

        # mAP@50-95 - END


        # YOLO-style Metrics - START
        iou_threshold = 0.5
        pryolo_classwise = defaultdict(lambda: {'TP': 0, 'FP': 0, 'FN': 0})
        ...
        for cls, stats in pryolo_classwise.items():
            ...
            pryolo_metrics[f'class_{cls}_Precision'] = prec
            pryolo_metrics[f'class_{cls}_Recall'] = rec
            pryolo_metrics[f'class_{cls}_TP'] = TP
            pryolo_metrics[f'class_{cls}_FP'] = FP
            pryolo_metrics[f'class_{cls}_FN'] = FN


        for k in pryolo_metrics:
            print(f'[YOLO-style metrics] {k}:{pryolo_metrics[k]}')    
        # YOLO-style Metrics - END
